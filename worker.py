...
v1: a simple in-process ThreadPoolExecutor. Good enough for a single dev instance;
swap for Celery/RQ if this ever needs multiple workers/processes.
"""
...
# name -> callable(target_url) -> dict   (only implemented modules go here)
_MODULE_RUNNERS = {}
...
def submit_assessment_job(app, assessment_id: int) -> None:
    _executor.submit(_run_pipeline, app, assessment_id)


def _run_pipeline(app, assessment_id: int) -> None:
    with app.app_context():
        from database.models import db, Assessment, ModuleRun, Finding
        ...
        module_runs = (
            ModuleRun.query.filter_by(assessment_id=assessment_id).order_by(ModuleRun.id).all()
        )
        total = len(module_runs) or 1
        any_failed = False
        context = {}  # module_name -> output dict, so later modules can reuse earlier crawls
        ...
        for i, run_row in enumerate(module_runs, start=1):
            ...
            if run_row.name == "reporting":
                # reporting doesn't scan the target -- it summarizes everything
                # every other module already found, so it runs last and reads
                # from the DB instead of taking target_url like the others.
                try:
                    ...
                    run_row.raw_output = tb
                    any_failed = True
                    logger.error("assessment %s: report generation failed\n%s", assessment_id, tb)
                run_row.finished_at = datetime.utcnow()
                assessment.progress = int(i / total * 100)
                db.session.commit()
                continue

            run_fn = _MODULE_RUNNERS.get(run_row.name)
            if run_fn is None:
                ...
                try:
                    output = run_fn(assessment.target_url, context=context)
                    ...
                    db.session.commit()
                    logger.info(
                        "assessment %s: module '%s' completed, %d finding(s) recorded",
                        assessment_id, run_row.name, findings_added,
                    )
                except Exception:
                    run_row.status = "failed"
                    tb = traceback.format_exc()
                    ...
                    any_failed = True
                    logger.error(
                        "assessment %s: module '%s' failed\n%s", assessment_id, run_row.name, tb
                    )
        ...
        db.session.commit()
        logger.info(
            "assessment %s: pipeline finished with status=%s", assessment_id, assessment.status
        )
